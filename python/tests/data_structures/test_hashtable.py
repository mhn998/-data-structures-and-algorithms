...
def test_hash(hashtable):
    assert hashtable._buckets[hashtable.hash('age')].head.value == '23'
    assert hashtable.hash('delicious') == 221


def test_add(hashtable):
    hashtable.add('why','so serious')
    assert hashtable._buckets[hashtable.hash('why')].head.value == 'so serious'
